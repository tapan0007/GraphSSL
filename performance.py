...
def visualization_PCA(logits, labels):
    # Apply PCA to reduce the dimensionality of embeddings
    pca_emb = PCA(n_components=2).fit_transform(logits)

    # Visualize the embeddings
    plt.figure(figsize=(8, 8))
    plt.scatter(pca_emb[:, 0], pca_emb[:, 1], c=labels)
    plt.title("Graph Embedding Visualization - PCA")
    plt.show()

def visualization_TSNE(logits, labels):
    tsne_emb = TSNE(n_components=2).fit_transform(logits)

    # Visualize the embeddings
    plt.figure(figsize=(8, 8))
    plt.scatter(tsne_emb[:, 0], tsne_emb[:, 1], c=labels)
    plt.title("Graph Embedding Visualization - TSNE")
    plt.show()
...
def performance_metrics(model, g, features, labels, mask):
    ...
    # Visualize the embeddings
    visualization_TSNE(logits, labels)
    visualization_PCA(logits, labels)
...
def performance_metrics_ssl(classifier, model, g, features, labels, mask):
    ...
    # Visualize the embeddings
    visualization_TSNE(logits.detach().numpy(), labels)
    visualization_PCA(logits.detach().numpy(), labels)
...
def ssldegree_performance(g, features, train_ids, train_labels, test_labels, test_ids):
    ...
    ssldegree_f1, ssldegree_g_mean, ssldegree_auroc = performance_metrics_ssldegree(encoder, classifier, g, features,
...
def graphSSL_performance(g, features, train_ids, train_labels, test_labels, test_ids):
    ...
    graphSSL_f1, graphSSL_g_mean, graphSSL_auroc = performance_metrics_ssl(classifier, ssl_model, g, features,
                                                                           test_labels, test_ids)
...
if __name__ == '__main__':
    ...
    metrics = pd.DataFrame(data, columns=["F1 Score", "Geo Mean", "AUROC"],
                           index=["GCN", "GraphSAGE", "SSLDegree", "GraphSSL", "DGI"])
